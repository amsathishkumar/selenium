...
import com.cisco.spvgt.utils.jarfilehandler.JarfileHandler;
...
public class SeleniumUtilities {
	...
	private static Validate miscValidate = new Validate();
	private static JarfileHandler miscJarfileHandler = new JarfileHandler();
			...
			try {
				if (getDriver() == null
					...
					if (browser.equals("firefox")
						...
						LogHandler
								.info("Fire Fox Driver without addon configuration");
						driver = new FirefoxDriver();
					} else if (browser.equals("chrome"))

					{
						LogHandler.info("Chrome Driver configuration");
						JarFile jarFile = miscJarfileHandler
								.jarForClass(SeleniumUtilities.class);
						miscJarfileHandler.copyResourcesToDirectory(jarFile,
								"chrome", "src/it/resources/chrome");
						...
						String IEpath = (SeleniumUtilities.class).getResource(
								"").getPath();
						System.out.println(IEpath.substring(0,
								IEpath.length() - 31));
						...
						Assert.assertTrue("include the browswer variable...",
								false);
					}
				}
			} catch (Exception e) {
