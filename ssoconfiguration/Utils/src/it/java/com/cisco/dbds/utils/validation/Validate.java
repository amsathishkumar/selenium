		 ...
		 for(int i=0;i<(Dvalue.getGherkinRows().size() -1);i++)
		 {
			 ...
			 for(String svrvalue:Dvalue.topCells())
			 {
			     ...
			     String val=stvalue.get(i).get(svrvalue);
        			...
        			{
        				val=readsystemvariable(val);		
        
        			}
        		if (data)
        		{
        			//svrvalue = svrvalue.toLowerCase();
        			if (svrvalue.contains("TABLE_NAME"))
        				val = val.toLowerCase();
        			if (svrvalue.contains("COLUMN_DETAILS"))
        				val = val.toLowerCase();
        			System.out.println("svrvalue"+svrvalue);
        			//	val=val.toLowerCase();
        			        			//*
        		}	
        		else
